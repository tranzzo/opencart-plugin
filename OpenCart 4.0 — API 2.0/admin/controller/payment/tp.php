<?php
namespace Opencart\Admin\Controller\Extension\OcPaymentTP\Payment;
...
class TP extends \Opencart\System\Engine\Controller
{
    ...
    public function index(): void
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->response->redirect($this->url->link('extension/oc_payment_tp/payment/tp', 'user_token=' . $this->session->data['user_token']. '&type=payment', true));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/oc_payment_tp/payment/tp', 'user_token=' . $this->session->data['user_token'], true)
        );

        $data['action'] = $this->url->link('extension/oc_payment_tp/payment/tp', 'user_token=' . $this->session->data['user_token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/oc_payment_tp/payment/tp', $data));
    ...
    {
        if (!$this->user->hasPermission('modify', 'extension/oc_payment_tp/payment/tp')) {
    ...
    public function order_list(&$route, &$data, &$output)
    {
        if(!empty($data['orders'])){
            ...
            foreach ($data['orders'] as $key => $order){
                ...
                $transactionsData = $this->model_extension_oc_payment_tp_payment_tp->searchTransactionsByOrderId($order_id);
                if(!empty($transactionsData)) {
                    $transactions = $this->load->view(
                        'extension/oc_payment_tp/payment/tp_transactions',
                        array(
                ...
                $paymentData = $this->model_extension_oc_payment_tp_payment_tp->getPaymentData($order_id);
                ...
                if(isset($paymentData['is_test'])){
                    ...
                    if($is_test){
                        $data['orders'][$key]['date_modified'] .= '<p><span style="display:block;width:100%;" class="badge bg-success">Test</span></p>';
                    }
                }
            }
        }
    ...
    public function order_info(&$route, &$data, &$output)
    {
        ...
        $this->load->model('extension/oc_payment_tp/payment/tp');
        $order = $this->model_extension_oc_payment_tp_payment_tp->getOrder($order_id);
        ...
        if ($order && $order['tp_payment']) {
            ...
            if (!empty($metaData)) {
                ...
                if(isset($metaData['payment_id']) && !empty($metaData['payment_id'])) {
                    ...
                    $tab['title'] = 'Refund';
                    $tab['code'] = 'tp_refund';
                    $total_refunded = $this->model_extension_oc_payment_tp_payment_tp->getTotalRefundedAmount($order_id);
                    ...
                    $content = $this->load->view('extension/oc_payment_tp/payment/tp_refund', $params);
                    ...
                    $tabTransactions['content'] = $this->load->view(
                        'extension/oc_payment_tp/payment/tp_transactions',
                        array(
                            'transactions' => $this->model_extension_oc_payment_tp_payment_tp->searchTransactionsByOrderId($order_id),
                            'currency_code' => !empty($this->currency->getSymbolRight($order_info['currency_code'])) ?
                                $this->currency->getSymbolRight($order_info['currency_code']) : $this->currency->getSymbolLeft($order_info['currency_code']),
                        )
                    );
                    ...
                    if (!empty($order_id)) {
                            ...
                            try {
                                ...
                                if ($paymentStatus) {
                                    if (isset($paymentStatus['method'])) {
                                        $this->model_extension_oc_payment_tp_payment_tp->updatePaymentData($order_id, 'payment_type', $paymentStatus['method']);
                                        $this->model_extension_oc_payment_tp_payment_tp->updatePaymentData($order_id, 'fees', $paymentStatus['fee']);
                                    }
                                }
                            } catch (\Exception $e) {
                        ...
                        if (!empty($payment_method)) {
                            ...
                            $data['totals'][] = array('title' => $this->globalLabel.' Fee', 'text' => $this->currency->format((float)$fees, $order_info['currency_code'], $order_info['currency_value']));
                        }
                    }
                }
            }
    ...
    public function refund()
    {
        ...
        try {
            ...
            $metaData = (array)json_decode($this->model_extension_oc_payment_tp_payment_tp->getPaymentData($order_id));
            $this->ServiceApi->writeLog(array('$payment_data', $metaData));

            if (!empty($metaData)) {

                if($metaData[ServiceApi::P_REQ_METHOD] == 'auth'){
                    $totalHold = $this->model_extension_oc_payment_tp_payment_tp->getTotalHoldAmount($order_id);
                    if($amount < $totalHold) {
                ...
                if($metaData[ServiceApi::P_REQ_METHOD] == 'capture'){
                    $captured = $this->model_extension_oc_payment_tp_payment_tp->getTotalCapturedAmount($order_id);
                    if($amount > $captured) {
                ...
                if (!isset($response["status"]) || $response["status"] != "success") {
                    ...
                    $this->model_extension_oc_payment_tp_payment_tp->createTransaction($response[ServiceApi::P_REQ_METHOD], $amount_payment, $order_id);

                    $total_refunded = $this->model_extension_oc_payment_tp_payment_tp->getTotalRefundedAmount($order_id);
                    if ($total_refunded >= $order_total_paid) {
                        $order = $this->model_extension_oc_payment_tp_payment_tp->updatePaymentData($order_id, 'is_refunded', 1);
    ...
    {
        $this->load->model('extension/oc_payment_tp/payment/tp');
        $this->model_extension_oc_payment_tp_payment_tp->install();
    }
    ...
    {
        $this->load->model('extension/oc_payment_tp/payment/tp');
        $this->model_extension_oc_payment_tp_payment_tp->uninstall();
    }
}
